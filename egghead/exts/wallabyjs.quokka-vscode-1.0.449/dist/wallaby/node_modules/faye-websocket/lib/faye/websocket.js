...
var WebSocket = function(request, socket, head, supportedProtos, options) {
  ...
  ['close', 'end', 'error'].forEach(function (event) {
    self._stream.addListener(event, function () {
      var attemptsLeft = closeAttempts[event] = closeAttempts[event] - 1;
      // On windows closing PhantomJs 2 causes a long chain of socket `error` calls
      // that are caused by a close attempts,
      // so need to cap the number of close attempts
      if (attemptsLeft >= 0) {
